...
namespace Sample {
	...
	/// ■ タイトルシーンのクラス
	/// </summary>
	public class SampleTitleSMScene : MainSMScene {

		/// <summary>
		/// ● コンストラクタ
		/// </summary>
		public SampleTitleSMScene() {
			...
			inputManager.GetKey( SMInputKey.Decide )._enabledEvent.AddLast()
				.Where( _ => _isEntered )
				.Subscribe( _ => {
					_owner.GetFSM<MainSMScene>().ChangeState<SampleGameSMScene>().Forget();
				} );
			...
			_enterEvent.AddLast( async canceler => {
				SMLog.Debug( $"Enterキーでゲームシーンへ、ESCキーでアプリを終了します。" );
